...
class StartAutoConfigureRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            $policy = (string) $this->input('host_key_policy', 'strict_tofu');
            $fingerprint = trim((string) $this->input('host_fingerprint', ''));
            if ($policy === 'strict_pinned' && $fingerprint === '') {
                $validator->errors()->add('host_fingerprint', 'host_fingerprint is required when host_key_policy is strict_pinned.');
            }
        });
    }
}
