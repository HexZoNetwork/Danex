...
def main() -> int:
    ...
    assert_contains(text, '(websocket|resources|activity)', "HTTP access scoring must ignore normal server polling lanes by default")
    assert_contains(text, '^/api/client/chat(/|$)', "HTTP access scoring must ignore chat lanes by default")
    assert_contains(text, '^/api/client/rum(/|$)', "HTTP access scoring must ignore RUM lanes by default")
    assert_contains(text, 'status !~ /^[23][0-9][0-9]$/', "HTTP access scoring should only count accepted 2xx/3xx requests")
    assert_contains(text, "extract_access_limiter_ip_counts()", "access-log limiter extraction missing")
    assert_contains(text, 'status ~ /^(429|461|463|503)$/', "access-log limiter extraction must count challenge/rate-limit statuses")
    assert_contains(high, "access-limiter:*", "access limiter reason must be high-confidence after repeated 429 threshold")
    assert_contains(text, 'nginx-limiter:*|access-limiter:*) return 0', "access limiter reason must be metric-block classified")
    assert_contains(text, 'nginx-limiter:*|access-limiter:*) printf \'68\'', "access limiter reason must use limiter confidence score")
    assert_contains(text, 'ACCESS_LIMITER_REQ_THRESHOLD="$(clamp_min_int "$(read_network_setting access_limiter_req_threshold 6)" 2)"', "access-log 429 limiter threshold must default lower than error-log limiter")
    assert_contains(text, 'detect_limiter_abusers "${limiter_access_ip_counts}" "${ACCESS_LIMITER_REQ_THRESHOLD}" "access-limiter"', "access-log limiter hits must use the lower access threshold")
    assert_contains(text, 'limiter_ip_counts="$(printf', "limiter counts must merge nginx error-log and access-log signals")
    assert_contains(text, "merge_ip_count_sets", "limiter counts must be merged before mitigation")
    assert_contains(config, '"access_limiter_req_threshold": 6', "config should expose repeated 429 ban threshold")
